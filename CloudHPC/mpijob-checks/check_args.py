...
MPI_YAML_FILE_NAME = "hpl-mpi.yaml"
HPL_DAT_FILE_NAME = "HPL.dat"
...
def check_slots_per_worker(yaml_meta): 
    ...
    if slots_per_worker != num_cpus_req: 
        print(f"{bc.FAIL} Slots per worker should equal number of cpus requested!!{bc.ENDC}")
...
def check_np_vs_PQ(yaml_meta, hpl_configs): 
    try: 
        ...
        np_stated = yaml_meta["spec"]["mpiReplicaSpecs"]["Launcher"]["template"]["spec"]["containers"][0]["command"][0]
       
        np_stated = np_stated.split()
        np_stated = int(np_stated[np_stated.index("-np") + 1])
